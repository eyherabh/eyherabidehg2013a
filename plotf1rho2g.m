function hfig=plotf1rho2g(example)
    ...
    % of the correlation coefficient of responses elicited by stimulus S2, 
	% and constructs a panel like Fig 1I.
    %
    ...
    %   ps1: Probability of stimulus S1. Must lie between 0 and 1.
    ...
    if ~isfield(example,'ps1'), errmessage=[errmessage, 'The field "ps1" corresponding to the probability of stimulus S1 is missing']; end
    ...
    if example.ps1>1 || example.ps1<0, errmessage=[errmessage, 'The value of ps1 must lie between 0 and 1\n']; end
    if ~isempty(errmessage), error('infolossgauss:errorinput',errmessage); end
    ...
    for rho2=-0.95:.05:.95,
        ...
        info=informationgauss(example.ps1,example.mean1,example.mean2,example.rho1,rho2);
        
        % Estimates the minimum information loss
        for itype=1:3,
            di(itype,indrho)=100*infolossgauss(ditype{itype},example.ps1,example.mean1,example.mean2,example.rho1,rho2)/info;
        end
    end
    ...
    for itype=1:3,
        plot(-.95:.05:.95,di(itype,:),'o',options{itype,:});
        hold on;
    end
